...
public class IdentityServiceApplication extends Application<IdentityServiceConfiguration> {
    ...
    public void run(IdentityServiceConfiguration configuration, Environment environment) throws ClassNotFoundException {
        ...
        final WorkIdentifierDAO workIdentifierDAO = jdbi.onDemand(WorkIdentifierDAO.class);
        final PersonDAO personDao = jdbi.onDemand(PersonDAO.class);
        environment.jersey().register(new ClaimResource(claimDao, personDao, identifierDao, nameDao, workDao, workIdentifierDAO));
        environment.jersey().register(new NameResource(nameDao, personDao, claimDao, workDao));
        environment.jersey().register(new PersonResource(personDao, identifierDao, claimDao));
        environment.jersey().register(new WorkResource(workDao, workIdentifierDAO, personDao, claimDao));
